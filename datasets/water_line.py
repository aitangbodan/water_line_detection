...
class Wsoc11(BaseDataset):
    def __init__(self, 
        ...
        super(Wsoc11, self).__init__(ignore_label, base_size,
        ...
        self.color_list = [[0, 0, 0], [255, 255, 255], [64, 0, 128],
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image = Image.open(os.path.join(self.root,'wsoc11',item["img"])).convert('RGB')
        image = np.array(image)
        size = image.shape

        color_map = Image.open(os.path.join(self.root,'wsoc11',item["label"])).convert('RGB')
