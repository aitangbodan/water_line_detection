...
def parse_args():
    ...
    parser.add_argument('--cfg',
                        ...
                        default="configs/wsoc11/pidnet_m_wsoc11.yaml",
                        type=str)
...
def collate_fn(batch):
    ...
    for _, sample in enumerate(batch):
        img, label, edge, size, name = sample
        imgs.append(torch.from_numpy(img))
        labels.append(torch.from_numpy(label))
        edges.append(torch.from_numpy(edge))
        sizes.append(torch.from_numpy(size))
        names.append(name)
    try:
    ...
    return (imgs, labels, edges, sizes, names)
...
def main():
    ...
    if config.TRAIN.OPTIMIZER == 'sgd':
        ...
        raise ValueError('Only Support SGD, Adam, AdamW optimizer')
